...
function processData(rows, callback) {
    ...
    const raw = values[8][0].split(";");
    const date = [raw[5], raw[6], raw[7]];
    ...
    Object.keys(values).map(k =>{
        ...
        response[k] = {...day[hour], ICA: Number(ica.toFixed(2)), ica_p, date_p: getDate([...date, hour])};
    });
...
function getDate(date) {
    ...
    return `${date[0]}-${date[1]}-${date[2].length===1 ? "0"+date[2] : date[2]}T${String(date[3]).length===1 ? "0"+date[3] : date[3]}:00:00`;
}
